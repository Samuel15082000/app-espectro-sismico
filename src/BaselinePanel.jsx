...
export default function BaselinePanel({ accelArr, dt, fileName, onClose, onUseCorrecta, unitFactor = 1, unitLabel = 'cm/s²' }) {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i < n; i += step) {
      const pt = {
        ...
        orig: parseFloat((accelArr[i] / unitFactor).toFixed(5)),
      }
      if (result) pt.corr = parseFloat((result.corrected[i] / unitFactor).toFixed(5))
      data.push(pt)
    }
    return data
  }, [result, accelArr, dt, unitFactor])
        ...
        <div style={{ display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }}>
          ...
          <div style={{ width: 210, minWidth: 210, borderRight: `1px solid ${BORDER}`, padding: 14, display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'auto' }}>
            ...
            {result && (<>
              ...
              <button onClick={() => exportBaselineTxt(accelArr, result.corrected, result.poly, dt, result.coef, result.order, result.stats, fileName, 1 / unitFactor, unitLabel)} style={{ padding: '8px', borderRadius: 5, border: `1px solid ${BORDER}`, background: '#21262D', color: '#ccc', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>
                Descargar .txt
              </button>
            </>)}
          </div>
              ...
              <ResponsiveContainer width="100%" height={360}>
                <LineChart data={chartData} margin={{ top: 4, right: 10, left: 0, bottom: 20 }}>
                  ...
                  <YAxis stroke="#21262D" tick={{ fontSize: 10, fill: '#8B949E' }}
                    label={{ value: `a (${unitLabel})`, angle: -90, position: 'insideLeft', fill: '#8B949E', fontSize: 10, dy: 30 }} />
